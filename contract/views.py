...
def generate_internal_code(request:HttpRequest):
    '''Обработкчик AJAX для генерации внутреннего кода'''
    try:
...
def check_inn(request:HttpRequest):
    '''AJAX обраточик для проверки ИНН на дубликаты'''
    try:
        data = json.loads(request.body)
...
def duplicate_groups(request:HttpRequest):
    ...
    if request.method == 'POST':
        ...
        form = BullDeleteForm()
    counterparties_with_checkboxes =[]
    for inn,counterparties in duplicate_groups_data.items():
        for cp in  counterparties:
            counterparties_with_checkboxes.append({
                'id': cp.internal_code,
                'name': cp.name,
                'inn': cp.inn,
                'deletion_mark': cp.deletion_mark
            })
    context = {
